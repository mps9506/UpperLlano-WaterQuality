...
- name: Tom Arsuffi
  affiliation: Texas Tech University, Llano River Field Station
abstract: "Trends in *Escherichia coli* (*E. coli*) and dissolved oxygen at water quality monitoring stations in the Upper Llano watershed were evaluated for calendar years 2001-2016. Trends in *E. coli* concentrations and loads were evaluated with linear regressions and Weighted Regressions on Time, Discharge, and Season (WRTDS) method. Trends in dissolved oxygen concentrations were evaluated with linear regression and generalized additive mixed models (GAMMs). Linear regressions provide insight on instream concentration trends. WRTDS and GAMMs were also used to remove effects of year to year variations of flow on *E. coli* and annual variations in mean temperature on dissolved oxygen. Linear regression indicated a statistically significant decrease in *E. coli* on the main stem of the Llano River immediately downstream of the the North and South Llano River confluence. Linear regression did not indicate statistically significant changes in DO at any sites in the the watershed."
...
Figure 6 depicts the WRTDS estimated flow-normalized bacteria concentrations and loads at LR2 and NL1. From 2001 through 2016 annual mean flow-normalized *E. coli* concentration decreased 33 cfu/100 mL and flow normalized loads decreased 5.87×10^6^ million colonies per day from 2001 through 2016 at LR2 (Table \@ref(tab:table4), Figure \@ref(fig:figure6)). *E. coli* concentration and load at NL1 decreased by 235 cfu/100mL and 1.27 × 10^5^ million colonies per day from 2002 through 2016. Although substantial, the estimates for both stations exhibit considerable uncertainty with estimated bias statistic of 0.287 for LR2 and 0.138 for NL1. An inspection of model diagnostics indicate samples at both stations are considerably underrepresented at high flow events. Furthermore, temporal gaps (2009-2011) in monitoring data decrease confidence in these estimated concentration and loads.
...
While standard linear regression did not indicate a trend in dissolved oxygen concentrations, the GAMM regression indicated a small but significant decrease in dissolved oxygen as a function of time controlled for temperature and season. Overall dissolved oxygen concentration remained in compliance over the sampling period; however, the temperature and seasonal controlled trends point to possible underlying changes in mean dissolved oxygen concentration. 
Importantly, for NL1, periodic seasonal exceedances of the dissolved oxygen water quality standard appear more likely to occur based on the GAMM regression (Figure \@ref(fig:figure13)). Again, the lack of recent sampling data negatively impacts the confidence of regression results near the end of the sampling period, as reflected in the wide standard errors at NL1 (Figure \@ref(fig:figure11)). Increased monitoring frequency and continuity in site sampling effort would certainly increase confidence in regression results.
While changes in DO concentrations are informative, understanding the potential drivers and mechanisms behind those changes would be valuable for land management decisions. Crucially, covariates such as nitrogen and phosphorous loads were not estimated because of the sparse sampling efforts associated with nutrients parameters thus far.

# Conclusion

This report describes trends in *E. coli* and DO in the Upper Llano watershed using regression-based techniques (WRTDS and GAMM). Examination of the trends indicate that the parameters are within acceptable ranges for compliance with existing water quality standards. However, the GAMM approach identified a decreasing trend in dissolved oxygen concentrations (controlled for season and temperature) at NL1. Caveats to this report include substantial bias identified in the *E. coli* load estimations attributed to sparse data collection and lack of high flow sampling events. In order to provide increasingly reliable load estimates, future sampling efforts need to ensure representativeness of sampling conditions (flow and season). Furthermore, while the GAMM estimates of DO were reasonable, regressing dissolved oxygen changes with changes in nutrient loads would be informative for land management purposes. However, the sparseness of instream nutrient data collected to date prevents such analysis. Therefore additional efforts to fill this data gap are also recommended.